...
def translation_example():
    ...
    result = client.translation.create(
        text="Hello, how are you?", source_lang="eng_Latn", target_lang="hin_Deva"
    )
...
def tts_example():
    ...
    audio = client.audio.speech.create(text="नमस्ते दुनिया", lang="hi", speaker="female", format="wav")
...
def stt_example():
    ...
    # From file path
    try:
        transcription = client.audio.transcriptions.create(audio="audio.wav", lang="hi")
...
def transliteration_example():
    ...
    result = client.transliteration.create(
        text="namaste", source_script="latn", target_script="deva", lang="hi", topk=3
    )

    print("Original: namaste")
...
def error_handling_example():
    ...
    from zaban import AuthenticationError, ValidationError, ZabanError

    try:
        # This will fail with an invalid API key
        invalid_client = Zaban(api_key="sk-invalid")
        invalid_client.translation.create(text="Hello", target_lang="hin_Deva")
